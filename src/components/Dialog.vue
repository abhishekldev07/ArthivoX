      ...
      <div v-if="open" class="backdrop z-20 flex justify-center items-center">
        ...
        >
          <div class="flex justify-between items-center gap-3 min-w-0">
            <h1 class="font-semibold min-w-0 flex-1 arthivox-dialog-title">
              {{ title }}
            </h1>
            <FeatherIcon
          ...
          <template v-if="detail">
            <p
              ...
              class="text-base arthivox-dialog-detail"
            >
              {{ detail }}
            </p>
            ...
            <div
              v-else
              v-for="(d, index) of detail"
              :key="index"
              class="min-w-0 max-w-full"
            >
              <p class="text-base arthivox-dialog-detail">{{ d }}</p>
            </div>
          </template>

          <div class="flex justify-end gap-3 mt-4 flex-wrap min-w-0 max-w-full">
            <Button
              v-for="(b, index) of buttons"
              :ref="b.isPrimary ? 'primary' : 'secondary'"
              :key="b.label"
              style="min-width: 5rem; max-width: 100%"
              :type="b.isPrimary ? 'primary' : 'secondary'"
              class="arthivox-dialog-button"
...
.arthivox-dialog-title,
.arthivox-dialog-detail,
.arthivox-dialog-button {
  min-width: 0;
  max-width: 100%;
  white-space: normal;
