...
const ipc = {
  ...
  // ArthivoX cloud-backup bridge. This must live inside the object exposed
  // through contextBridge.exposeInMainWorld('ipc', ipc).
  async createDbSnapshot(): Promise<Uint8Array> {
    const data = await ipcRenderer.invoke(IPC_ACTIONS.CREATE_DB_SNAPSHOT);
  ...
  async saveBinaryData(data: Uint8Array, savePath: string) {
